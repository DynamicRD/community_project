  ...
  if (loading) {
    ...
    return (
      <Link to={`/group/detail?group_no=${item.GROUP_NO}`} style={{ textDecoration: 'none', color: 'inherit' }}> 
        <div className="col">
          <div className="card">
            <img src={`http://localhost:8080/upload/${item.IMG_URL1}`}className="card-img-top img-fluid p-3" 
              style={{height:"264.83px", width:"394px"}} alt="..."/>
            <div className="card-body">
              <h5 className="card-title"><strong>{item.GROUP_TITLE}</strong><span>
                </span></h5>
              <p className="card-text">{formattedStartDate} / {item.AREA} / {item.CATEGORY === 'culture' && '문화/예술'}
              {item.CATEGORY === 'food' && '푸드/드링크'}
              {item.CATEGORY === 'edu' && '교육'}
              {item.CATEGORY === 'travel' && '여행'}
              {item.CATEGORY === 'hobby' && '취미'}</p>
            </div>
          </div>
        </div>
      </Link>
